...
function calculate_average(results, profiles) {
  for (const entry of results) {
    if (
      entry.folder === "Forms" &&
      entry.list === "ClickUp QA Audit Form"
    ) {
      for (const task of entry.tasks) {
        ...
        const ratingField = task.custom_fields.find(
          f => f.name === "Overall Compliance Rating to the Policy"
        );
...
router.get("/", async (req, res) => {
  try {
    ...
    //pipeline
    const profiles = create_profiles(results);
    const answer = calculate_average(results, profiles);
